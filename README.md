...
| **3. 데이터 통합** | 두 자료에서 대응하는 항목을 자동 제시 — 이름이 달라도 찾아낸다(`데이터기준일` ↔ `기준연월`). 값 표기도 하나로 묶고, **묶이지 않은 3곳은 이유까지** 밝힌다 |
...
- **이름이 다른 같은 항목을 어떻게 찾는가** — 소스↔표준(화면 7)과 자료↔자료(화면 3)가 같은 메커니즘이다. 컬럼 유사도로 후보를 뽑고, 값 표기는 별도로 정규화한다
...
[분석]  AI 컬럼 연관성 분석 — Transformer 임베딩 + XGBoost 타입 분류를 결합해
        ...
        값 표기 정규화는 별도로 — 유사도 행렬 + DBSCAN 으로 "성남시 / 경기도 성남시"를 한 개체로 묶음
```
